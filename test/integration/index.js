...
describe('integration of output evaluators', function () {
  ...
  describe('generation of an array of evaluators', function () {
    it('should be able to generate a single evaluator with a callback', done => {
      GENERATE({ segments: [ MOCKS.DEFAULT, MOCKS.ERROR ] }, (err, evaluators) => {
        try {
          if (err) throw err;
          else {
            expect(evaluators).to.have.property('generic_text_and_email');
            expect(evaluators).to.have.property('generic_text_and_email_1');
            done();
          }
        } catch (e) {
          done(e);
    ...
    it('should be able to generate a single evaluator and return a Promise', done => {
      GENERATE({ segments: [ MOCKS.DEFAULT, MOCKS.ERROR ] })
        .try(evaluators => {
          expect(evaluators).to.have.property('generic_text_and_email');
          expect(evaluators).to.have.property('generic_text_and_email_1');
          done();
